...
class RequestView(QFrame):
  ...
  def setup_ui(self):
    ...
    if(self.request.status == "pending"):
      ...
      self.return_button.clicked.connect(lambda: self.return_request())

      self.button_frame_layout.addWidget(self.return_button)
    else: 
      
      self.button_frame_layout.addWidget(self.return_button)
      
      self.status_label = QLabel("Status: " + self.request.status)
      self.button_frame_layout.addWidget(self.status_label)
  ...
  def return_request(self):
    Request.return_request(self.request.id)
    QMessageBox.about(self, "Success", "Request Returned")
    new_dashboard = dashboard.Dashboard(self.parent)
    self.parent.replaceWidget(self.parent.itemAt(0).widget(), new_dashboard)
    self.parent.setCurrentIndex(0)